...
const Insurance = () => {
  ...
  const submitInsuranceForm = async (
    ...
    try {
      ...
      const supplierState = sanitizeText(resolvedFormData.supplierState || "");
      const supplierDistrict = sanitizeText(
        resolvedFormData.supplierDistrict || "",
      );
      const supplierMandi = sanitizeText(resolvedFormData.supplierMandi || "");
      const buyerState = sanitizeText(resolvedFormData.buyerState || "");
      const buyerDistrict = sanitizeText(resolvedFormData.buyerDistrict || "");
      const buyerMandi = sanitizeText(resolvedFormData.buyerMandi || "");
      const supplierLocationLine = sanitizeText(
        buildMandiLocationLine({
          state: supplierState,
      ...
      const buyerLocationLine = sanitizeText(
        buildMandiLocationLine({
          state: buyerState,
      ...
      const placeSupply = sanitizeText(
        (isCash ? buyerState : supplierState) ||
      ...
      if (shouldRequireVerifiedParties) {
        if (isCash) {
          if (!selectedBuyerId) {
            throw new Error(
              "Insured party/buyer must be selected from verified users.",
            );
          }
          ...
          if (!selectedSupplierId) {
            throw new Error(
              "Insured party/supplier must be selected from verified users.",
            );
          }
      ...
      submitData.append(
        "invoiceType",
        isCash ? "BUYER_INVOICE" : "SUPPLIER_INVOICE",
      );
      ...
      if (resolvedFormData.notes)
        submitData.append(
          "weighmentSlipNote",
          sanitizeText(resolvedFormData.notes),
        );
      ...
      if (["CUSTOMER", "TRANSPORTER"].includes(identity)) {
        ...
        const insuredUserId = isCash ? selectedBuyerId : selectedSupplierId;
      ...
      if (rawPdfUrl) {
        ...
        if (isBotEmbed) {
          ...
          const target =
            user?.identity === "AGENT" ? "/agent/dashboard" : "/home";
  ...
  const handleEdit = (fieldToEdit: string) => {
    ...
    if (fieldToEdit === "language") {
      ...
      fieldToEdit !== "weightmentSlip" &&
      fieldToEdit !== "invoiceDate"
    ) {
  ...
  const getQuestionText = (question: Question, latestNotes?: string) => {
    ...
    if (question.field === "buyerState") {
      return isCashModeForQuestion
        ? "Insured Party / Buyer State"
        : "Buyer State";
    }
    if (question.field === "buyerDistrict") {
      return isCashModeForQuestion
        ? "Insured Party / Buyer District"
        : "Buyer District";
    }
    if (question.field === "buyerMandi") {
      return isCashModeForQuestion
        ? "Insured Party / Buyer Mandi"
        : "Buyer Mandi";
    }
    if (question.field === "supplierState") {
      return isCashModeForQuestion
        ? "Supplier State"
        : "Insured Party / Supplier State";
    }
    if (question.field === "supplierDistrict") {
      return isCashModeForQuestion
        ? "Supplier District"
        : "Insured Party / Supplier District";
    }
    if (question.field === "supplierMandi") {
      return isCashModeForQuestion
        ? "Supplier Mandi"
        : "Insured Party / Supplier Mandi";
    }
  ...
  const getInsuredPartyPhoneDefault = (snapshot: Partial<FormData> = {}) => {
    ...
    if (isCash) {
      const buyerName = String(mergedForm.buyerName || "")
        .trim()
        .toLowerCase();
      const buyerPhone =
  ...
  const currentLookupUsesVerified =
    (isSupplierLookupQuestion &&
      shouldRequireVerifiedParties &&
      String(formData.notes || "").toLowerCase() !== "cash") ||
    (isBuyerLookupQuestion &&
      shouldRequireVerifiedParties &&
      String(formData.notes || "").toLowerCase() === "cash");
        ...
        <div className="fixed inset-0 z-50 bg-black flex flex-col">
          ...
          <div className="w-full bg-black/90 p-4 pb-8 flex justify-between items-center px-6 shrink-0 z-50">
            ...
            <div className="flex items-center gap-4">
              ...
              >
                <div
                  className={`p-2 rounded-full bg-gray-800 border ${isCropperReady ? "border-[#25D366]" : "border-gray-500"} hover:bg-gray-700`}
                >
            ...
            <div className="flex items-center gap-2 max-w-[80%] sm:max-w-[75%]">
              {m.sender === "user" && m.field && !isSubmitting && (
                <button
                  ...
                  className={`p-2 rounded-full shadow-md transition-all duration-200 active:scale-95 ${
              ...
              <div
                className={`px-4 py-3 text-[15px] rounded-2xl shadow-lg transition-all duration-200 hover:shadow-xl ${
                  m.sender === "user"
        ...
        {showLocationConfirmation && currentLocationRole && (
          <div className="flex justify-start w-full animate-fadeIn">
            <div className="w-[80%] sm:w-[75%] bg-white rounded-2xl p-4 shadow-lg border-2 border-gray-100">
              <p className="text-xs text-gray-600 mb-3 font-semibold uppercase tracking-wider">
              ...
              <div className="flex flex-wrap gap-2.5">
                <button
                  ...
                  className="bg-gradient-to-r from-[#dcf8c6] to-[#d4f0b8] border-2 border-[#25D366] text-gray-900 px-4 py-2.5 rounded-xl text-sm font-semibold shadow-md transition-all duration-200 active:scale-95 flex-1 min-w-[130px]"
                >
                  {language === "hi" ? "Confirm" : "Confirm"}
                </button>
                <button
                  type="button"
                  onClick={() => handleLocationModify(currentLocationRole)}
                  className="bg-gradient-to-r from-white to-gray-50 border-2 border-gray-200 text-gray-800 px-4 py-2.5 rounded-xl text-sm font-medium shadow-md hover:from-[#dcf8c6] hover:to-[#d4f0b8] hover:border-[#25D366] transition-all duration-200 active:scale-95 flex-1 min-w-[130px]"
                >
                  {language === "hi" ? "Modify" : "Modify"}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="bg-gradient-to-t from-[#f0f0f0] to-[#f5f5f5] px-4 py-3 border-t border-gray-300 shadow-lg z-10 shrink-0">
          ...
          {isFileInput ? (
            ...
            <form onSubmit={handleSubmit} className="flex items-center gap-3">
              ...
              <button
                ...
                className={`p-3 rounded-full text-white shadow-lg transition-all duration-200 active:scale-95 min-w-[48px] flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed ${
                  editingMessageIndex !== null
          ...
          {!isFileInput && (
            <p className="text-xs text-gray-500 mt-2 text-center">
              💡{" "}
              {language === "hi"
                ? "टिप: अपना उत्तर टाइप करें और भेजें बटन दबाएं"
                : "Tip: Type your answer and press send"}
            </p>
          )}
        </div>
      )}

...
test("does not intercept browser shortcuts, modified keys, or composition", () => {
  assert.equal(getCalculatorCommandFromKeyboard({ key: "c", metaKey: true }), null);
  assert.equal(getCalculatorCommandFromKeyboard({ key: "7", ctrlKey: true }), null);
  assert.equal(getCalculatorCommandFromKeyboard({ key: "/", altKey: true }), null);
  assert.equal(

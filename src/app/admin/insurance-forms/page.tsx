...
import { FileText, RefreshCw, Upload, Eye, CheckCircle, AlertCircle, X, XCircle, Pencil, ChevronDown, ChevronRight, MoreVertical, Link as LinkIcon, RotateCcw, CreditCard, QrCode, Link2 } from 'lucide-react';
...
export default function InsuranceFormsPage() {
    ...
    const handleRazorpayGenerateLink = async (invoiceId: string) => {
        ...
        try {
            ...
            toast.error(err?.message || 'Failed to generate Razorpay link');
        } finally {
            setRazorpayLoading(false);
        }
    };
                                                                        ...
                                                                        {!inv.isRejected && inv.isVerified && (
                                                                            <Menu.Item>
                                                                                {({ active }) => (
                                                                                    <button
                                                                                        type="button"
                                                                                        onClick={() => openRazorpayModal(getInvoiceId(inv))}
                                                                                        className={`${active ? 'bg-gray-100' : ''} flex w-full items-center gap-2 px-4 py-2 text-left text-sm text-gray-700`}
                                                                                    >
